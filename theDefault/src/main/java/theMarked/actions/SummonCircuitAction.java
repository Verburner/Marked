...
public class SummonCircuitAction extends AbstractGameAction{
    ...
    public SummonCircuitAction(AbstractCard card, int amount, boolean upgraded) {
        this.card = card;
        this.amount = amount;
    ...
    public void update() {
        ...
        for (AbstractCard c : AbstractDungeon.player.discardPile.group) {
            if (c.cardID == card.cardID && AbstractDungeon.player.hand.size() < BaseMod.MAX_HAND_SIZE && !found) {
                x = c;
                found = true;
                place = 1;
            }
