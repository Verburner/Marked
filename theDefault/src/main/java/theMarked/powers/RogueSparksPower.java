...
public class RogueSparksPower extends AbstractPower implements CloneablePowerInterface {
    ...
    public void onCardDraw(AbstractCard card) {
        if (card.type == AbstractCard.CardType.ATTACK) {
            ...
            if (m != null) {
                this.addToTop(new DamageAction(m, new DamageInfo(m, this.amount, DamageInfo.DamageType.THORNS), AbstractGameAction.AttackEffect.NONE, true));
                this.addToTop(new VFXAction(new LightningEffect(m.drawX, m.drawY), 0.0F));
                this.addToTop(new SFXAction("ORB_LIGHTNING_EVOKE"));
            }
            this.addToBot(new GainBlockAction(this.owner, this.amount2));
        }
    }

    public void stackPower(int stackAmount) {
        super.stackPower(stackAmount);
        if (stackAmount > 0) this.amount2 += stackAmount-1;
    ...
    public AbstractPower makeCopy() {
        return new RogueSparksPower(owner, source, amount, amount2);
    }
}
